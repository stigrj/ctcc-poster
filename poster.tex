...
\begin{equation}
    \Helmholtz{f}(r) = \int G(r-r')\Big[f(r')\Big]\ud r'
\end{equation}
...
\end{equation}
is written in integral form using the BSH operator\cite{Harrison}, 
which is the inverse of the shifted kinetic operator
\begin{equation}
    2\hat{G}   = \big[\kinetic-\epsilon_i\big]^{-1}
\end{equation}
...
\begin{equation}
    2\hat{G}^{(\pm)} = \Big[\hat{T} - (\epsilon \pm \omega)\Big]^{-1} 
\end{equation}
\begin{equation}
...
\begin{eqnarray}
    \pert{\coulomb}{1} \orbital_p &=& 
        \int\frac{\pert{\rho}{1}(r',r')\orbital_p(r)}{|r-r'|} \ud r' \\
    \pert{\exchange}{1} \orbital_p &=&
        \int\frac{\pert{\rho}{1}(r,r')\orbital_p(r')}{|r-r'|} \ud r' \\
    \pert{\xc}{1} \orbital_p &=& 
        \bigg[\frac{\delta^2 E_{xc}}{\delta \rho^2}
        \big[\pert{\rho}{0}\big]*\pert{\rho}{1}\bigg] \orbital_p
\end{eqnarray}
\end{textblock}
...
This is a test to see how long the line segment can be before we get a line
break. Hopefully this will happen soon, because I am running out of things to
write.
\end{textblock}
...
%LDA
	...
	MRChem $10^{-4}$ & 44.0935       & 45.2903       &  0.012256     &   8.1619      \\
	                 &               &               &               &               \\
	aug-cc-pVQZ      & 44.1326       &               &               &               \\
...
%	                 &               &               &               &               &               &               \\
%HF
	MRChem $10^{-2}$ &               &  -9.5497      &  -9.9222      &  446.7869     & -103.6166     &  343.1703     \\
